...
def toy_list():
    ...
    user_info = M_DB.user.find_one({"_id":ObjectId(user_id)})
    bind_toy = user_info.get("bind_toy")
    bind_toy_id = []
    for toy_id in bind_toy:
        bind_toy_id.append(ObjectId(toy_id))

    toys_list = list(M_DB.toys.find({"_id": {"$in": bind_toy_id}}))
...
def device_toy_id():
    ...
    if M_DB.devices.find_one({"device_id":device_id}):  # 玩具芯片存在
        toy_info = M_DB.toys.find_one({"device_id":device_id})  # 玩具一绑定用户
        if toy_info:
            RET["data"]["toy_id"] = str(toy_info.get("_id"))  # 玩具的id
            RET["data"]["audio"] = "success.mp3"  # 开机音乐
            return jsonify(RET)
        else:
            RET["msg"] = "找小主人"
